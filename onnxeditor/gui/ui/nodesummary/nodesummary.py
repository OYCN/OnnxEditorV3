...
from ....ir import OnnxNode, OnnxVar, OnnxGraph
    ...
    def getVal(self):
        ...
        if w is self._edit:
            ...
            if re.fullmatch(f"^-?\d+$", txt) is not None:
                ...
                "," in txt and re.fullmatch(f"^((-?[0-9a-zA-Z]+),?)*$", txt) is not None
            ):
...
class NodeSummary(QDialog):
    def __init__(
        self,
        nir: Union[OnnxNode, None] = None,
        g: Union[OnnxGraph, None] = None,
        ...
        self._ui.attr_tabel_widget.horizontalHeader().setSectionResizeMode(
        ...
        if nir is not None:
            ...
            for k, v in nir.attrs.items():
                self.addRow(k, v, isinstance(v, OnnxVar))
    ...
    def getRet(self):
        ...
        ret = {
            ...
            "inputs": [
                ins_w.itemWidget(ins_w.item(i)).text() for i in range(ins_w.count())
            ],
            "output": [
                outs_w.itemWidget(outs_w.item(i)).text() for i in range(outs_w.count())
            ],
            "attrs": {},
        }
