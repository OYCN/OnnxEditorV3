...
class ModelEditor(QDialog):
    def __init__(self, irm: OnnxModel, parent: Union[QWidget, None] = None) -> None:
        super().__init__(parent)

        vl = QVBoxLayout()

        self._key = {
            "ir_version": (QLineEdit(), int, 0),
            "producer_name": (QLineEdit(), str, ""),
            "producer_version": (QLineEdit(), str, ""),
            "domain": (QLineEdit(), str, ""),
            "model_version": (QLineEdit(), int, 0),
            "doc_string": (QLineEdit(), str, 0),
        }
