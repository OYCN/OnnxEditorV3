...
class GraphScene(QGraphicsScene):
    def __init__(self, ir: OnnxGraph, parent=None):
        super().__init__(parent)
        self._ir: OnnxGraph = ir
        self._normal_node = []
        self._io_node = []
        self._edge = []
        assert self._ir is not None

        def del_nodeitem(o: OnnxNode):
        ...
        def del_io_item(input: bool):
            def f(o: OnnxVar):
                n = o.read_ext("bind_gnode_src" if input else "bind_gnode_dst")
                n: GraphNode = o.read_ext("bind_gnode")
                n.setPos(0, 0)
                self.removeItem(n)
    ...
    def bind_node(self, ir: Union[OnnxNode, OnnxVar], asinput: bool = False):
    ...
    def bind_edge(self, ire: OnnxVar):
    ...
    def contextMenuEvent(self, event: QGraphicsSceneContextMenuEvent) -> None:
        ...
        m = QMenu()
        new_node_act = m.addAction("New OnnxNode")
    ...
    def addNodeDialog(self):
        ...
        dialog.setWindowTitle("Add OnnxNode")
        ret = dialog.exec()
        if ret == QDialog.DialogCode.Accepted:
            ret = dialog.getRet()
            return self.addNode(
                ret["name"], ret["op_type"], ret["inputs"], ret["output"], ret["attrs"]
            )
        return None
    ...
    def addIODialog(self, input=True):
        ...
        dialog = IOSummary(
            ...
            [
                v
                for v in self._ir.variables
                if v.isInput == (not input) and v.isOutput == input
            ],
        )
